    ...
    def fake_validate(schema, data):
        if data != {"ok": True}:
            raise JSONSchemaValidationError(
                code="schema_validation_failed", message="nope", errors=[]
            )
...
def test_validate_first_matching_raises_invalid_json_when_no_objects(monkeypatch):
    ...
    with pytest.raises(ex.AppError) as e:
        ex.validate_first_matching(schema, "no json here")

    assert e.value.code == "invalid_json"
    assert e.value.status_code == 422


def test_validate_first_matching_raises_schema_validation_failed_when_no_candidate_valid(
    monkeypatch,
):
    from llm_server.core.validation import JSONSchemaValidationError
    import llm_server.services.api_deps.extract.json_parse as ex
    ...
    def always_fail(schema, data):
        raise JSONSchemaValidationError(
            code="schema_validation_failed", message="bad", errors=[{"loc": "$"}]
        )
    ...
    schema = {"type": "object"}
    with pytest.raises(ex.AppError) as e:
        ex.validate_first_matching(schema, 'noise {"a": 1} {"b": 2}')
...
def test_failure_stage_mapping():
    ...
    assert failure_stage_for_app_error(e_parse, is_repair=False) == "parse"
    assert failure_stage_for_app_error(e_val, is_repair=False) == "validate"
    assert failure_stage_for_app_error(e_parse, is_repair=True) == "repair_parse"
    assert failure_stage_for_app_error(e_val, is_repair=True) == "repair_validate"
