...
def test_read_helpers_and_container_limit(monkeypatch):
    monkeypatch.setattr(
        lim, "get_settings", lambda: SimpleNamespace(max_concurrent_requests=200), raising=True
    )
    assert lim._read_max_concurrency() == 64
    monkeypatch.setattr(
        lim, "get_settings", lambda: SimpleNamespace(max_concurrent_requests="x"), raising=True
    )
...
def test_maybe_trip_memory_guard(monkeypatch):
    ...
    settings = SimpleNamespace(
        mem_guard_enabled=True, container_memory_bytes=1000, mem_guard_rss_pct=0.8
    )
    ...
    settings2 = SimpleNamespace(
        mem_guard_enabled=False, container_memory_bytes=1000, mem_guard_rss_pct=0.8
    )
...
async def test_guard_middleware_dispatch_paths(monkeypatch):
    ...
    monkeypatch.setattr(
        lim, "_maybe_trip_memory_guard", lambda request, settings: None, raising=True
    )
