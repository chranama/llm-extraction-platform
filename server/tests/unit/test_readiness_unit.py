...
async def test_model_ready_check_skips_when_load_mode_off(monkeypatch):
    monkeypatch.setattr(
        rd, "effective_model_load_mode_from_request", lambda request: "off", raising=True
    )
...
async def test_model_ready_check_skips_when_readiness_off(monkeypatch):
    monkeypatch.setattr(
        rd, "effective_model_load_mode_from_request", lambda request: "lazy", raising=True
    )
    monkeypatch.setattr(
        rd,
    ...
    monkeypatch.setattr(
        rd, "per_model_readiness_mode", lambda request, model_id: "off", raising=True
    )
...
async def test_model_ready_check_external_backend_missing(monkeypatch):
    ...
    monkeypatch.setattr(
        rd,
        "resolve_default_model_id_and_backend_obj",
        lambda request: ("m1", "remote", None),
        raising=True,
    )
    monkeypatch.setattr(
        rd, "per_model_readiness_mode", lambda request, model_id: "generate", raising=True
    )
...
async def test_model_ready_check_external_probe_paths(monkeypatch):
    ...
    monkeypatch.setattr(
        rd, "per_model_readiness_mode", lambda request, model_id: "probe", raising=True
    )
    ...
    monkeypatch.setattr(
        rd,
        "resolve_default_model_id_and_backend_obj",
        lambda request: ("m1", "llamacpp", backend),
        raising=True,
    )
    ...
    monkeypatch.setattr(
        rd,
        "resolve_default_model_id_and_backend_obj",
        lambda request: ("m2", "remote", backend),
        raising=True,
    )
...
async def test_model_ready_check_inproc_reflects_model_flags(monkeypatch):
    ...
    monkeypatch.setattr(
        rd,
        "resolve_default_model_id_and_backend_obj",
        lambda request: ("local-a", "transformers", object()),
        raising=True,
    )
    monkeypatch.setattr(
        rd, "per_model_readiness_mode", lambda request, model_id: "generate", raising=True
