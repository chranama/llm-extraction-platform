...
async def test_models_endpoint_deployment_and_model_caps_reflected(monkeypatch, client):
    monkeypatch.setattr(client.app.state.settings, "model_load_mode", "off", raising=False)
    ...
    monkeypatch.setattr(client.app.state.settings, "enable_generate", True, raising=False)
    monkeypatch.setattr(client.app.state.settings, "enable_extract", False, raising=False)

    monkeypatch.setattr(
        capabilities,
        "_model_capabilities_from_models_yaml",
        lambda model_id, request=None: {"extract": False} if model_id == "modelA" else None,
    )
