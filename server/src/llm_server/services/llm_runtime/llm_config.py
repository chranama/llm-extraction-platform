# server/src/llm_server/services/llm_runtime/llm_config.py
...
# Normalized-profile default (matches your semantics)
_DEFAULT_PROFILE = "host-transformers"
...
class ModelSpec:
    ...
    # Bool clamps used by runtime gating
    capabilities: Optional[CapabilitiesMap] = None

    # NEW: preserve raw capability shapes for observability (/modelz/admin)
    # - declared: what the model entry itself declares (no defaults applied)
    # - effective: defaults+overrides merged (what the runtime used to compute bool clamps)
    capabilities_declared: Optional[Dict[str, Any]] = None
    capabilities_effective: Optional[Dict[str, Any]] = None

    # stable key used to correlate server deployment <-> eval summary <-> policy decision
    deployment_key: Optional[str] = None

    # readiness semantics
    readiness_mode: Optional[ReadinessMode] = None

    # model/deployment permutations metadata (passed through from models.yaml)
...
def _canonicalize_requested_profile(name: str) -> str:
    """
    YAML uses normalized names only, but env can use common aliases.

    If you want strict behavior, set MODELS_STRICT_PROFILE_NAMES=1.
    """
    strict = (os.environ.get("MODELS_STRICT_PROFILE_NAMES") or "").strip() == "1"
    n = (name or "").strip()
    if not n:
...
def _select_profile(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Selection:
      ...
      Fallback to APP_PROFILE.
      Default: host-transformers.
    """
    ...
    requested = _canonicalize_requested_profile(requested)
...
def _normalize_readiness_mode(raw_value: Any, *, path: str, field: str) -> Optional[ReadinessMode]:
    if raw_value is None:
        return None
    v = _validate_enum(raw_value, field=field, path=path, allowed=_ALLOWED_READINESS_MODES)
    return cast(Optional[ReadinessMode], v)


def _normalize_deployment_key(raw_value: Any, *, path: str, field: str) -> Optional[str]:
    if raw_value is None:
        return None
    if not isinstance(raw_value, str):
        raise AppError(
            code="models_yaml_invalid",
            message=f"models.yaml {field} must be a string if provided",
            status_code=500,
            extra={"path": path, "field": field, "value": raw_value},
        )
    s = raw_value.strip()
    return s or None


def _normalize_meta_block(raw_block: Any, *, path: str, field: str) -> Optional[Dict[str, Any]]:
    return _normalize_backend_block(raw_block, path=path, field=field)
...
def _validate_capabilities_mapping(raw_caps: Any, *, path: str, field: str) -> Optional[Dict[str, Any]]:
    ...
    Allows values to be:
      - bool
      - dict (metadata; must contain enabled: bool to be used as a clamp)
    """
    if raw_caps is None:
        return None
    if not isinstance(raw_caps, dict):
        raise AppError(
            code="models_yaml_invalid",
            message=f"models.yaml {field} must be a mapping (dict) if provided",
            status_code=500,
...
def _cap_value_to_bool(v: Any, *, path: str, field: str, key: str) -> bool:
    """
    Convert a raw capability value to a bool clamp.

    Accepted shapes:
      - bool -> itself
      - dict -> must include enabled: bool
    """
    ...
    if isinstance(v, dict):
        ...
        raise AppError(
            code="models_yaml_invalid",
            message=f"models.yaml {field}.{key} object must include boolean 'enabled'",
            status_code=500,
            extra={"path": path, "field": f"{field}.{key}", "value": v},
        )
    raise AppError(
        code="models_yaml_invalid",
        message=f"models.yaml {field}.{key} must be a boolean or an object with 'enabled': boolean",
        status_code=500,
        extra={"path": path, "field": f"{field}.{key}", "value": v},
    )


def _deep_merge_capability_value(a: Any, b: Any) -> Any:
    """
    Merge a default value (a) with an override (b) for a single capability key.
    If both are dicts, deep-merge them (so policy metadata can accumulate).
...
def _merge_capabilities_raw(
    *,
    defaults_caps: Optional[Dict[str, Any]],
    declared_caps: Optional[Dict[str, Any]],
) -> Optional[Dict[str, Any]]:
    """
    Merge defaults + declared without losing object metadata.
    - declared overrides defaults per key
    - dict+dict deep merges
    """
    if defaults_caps is None and declared_caps is None:
        return None
    out: Dict[str, Any] = {}
    if isinstance(defaults_caps, dict):
        out.update(defaults_caps)
    if isinstance(declared_caps, dict):
        for k, v in declared_caps.items():
            if k in out:
                out[k] = _deep_merge_capability_value(out[k], v)
            else:
                out[k] = v
    return out


def _capabilities_raw_to_boolmap(
    raw_caps: Optional[Dict[str, Any]],
    *,
    path: str,
    field: str,
    ...
    out: Dict[str, bool] = {}
    for k, v in raw_caps.items():
        out[k] = _cap_value_to_bool(v, path=path, field=field, key=k)
    return cast(Optional[CapabilitiesMap], out)
...
def _normalize_model_entry(raw: Any, *, path: str, defaults: Dict[str, Any]) -> ModelSpec:
    if isinstance(raw, str):
        ...
        return ModelSpec(
            ...
            capabilities=effective_caps_bool,
            capabilities_declared=None,
            capabilities_effective=effective_caps_raw,
            deployment_key=defaults.get("deployment_key"),
            readiness_mode=defaults.get("readiness_mode"),
            deployment=dict(defaults.get("deployment") or {}) or None,
            assessment=dict(defaults.get("assessment") or {}) or None,
            backend_constraints=dict(defaults.get("backend_constraints") or {}) or None,
    ...
    return ModelSpec(
        ...
        load_mode=cast(LoadMode, load_mode),
        capabilities=effective_caps_bool,
        capabilities_declared=declared_caps_raw,
        capabilities_effective=effective_caps_raw,
        deployment_key=deployment_key,
        readiness_mode=readiness_mode,
        deployment=deployment_blk or None,
        assessment=assessment_blk or None,
        backend_constraints=constraints_blk or None,
...
def _cap_bool(caps: Optional[CapabilitiesMap], key: str, default: bool) -> bool:
    if not caps:
        return default
    v = caps.get(cast(Capability, key))
...
def _apply_effective_service_caps_from_primary(
    *,
    s: Any,
    primary: ModelSpec,
    defaults_caps_bool: Optional[CapabilitiesMap],
) -> Tuple[bool, bool]:
    gen_default = _cap_bool(defaults_caps_bool, "generate", True)
    ex_default = _cap_bool(defaults_caps_bool, "extract", False)
...
# -----------------------------
# Deployment key enforcement
# -----------------------------

_GENERIC_DEPLOYMENT_KEYS = {
    "default",
}


def _should_enforce_deployment_keys(*, selected_profile: Optional[str]) -> bool:
    if os.environ.get("ALLOW_GENERIC_DEPLOYMENT_KEY", "").strip() == "1":
        return False
    if os.environ.get("MODELS_ALLOW_GENERIC_DEPLOYMENT_KEY", "").strip() == "1":
        return False
    prof = (selected_profile or "").strip().lower()
    if prof == "test":
        return False
    return True
...
def _validate_deployment_keys_or_raise(*, specs: List[ModelSpec], path: str, selected_profile: Optional[str]) -> None:
    ...
    prof = (selected_profile or "").strip() or "unknown"
    bad: List[Dict[str, Any]] = []
    for sp in specs:
        dk = (sp.deployment_key or "").strip()
        if not dk:
            bad.append({"model_id": sp.id, "reason": "missing"})
            continue
        if dk.strip().lower() in _GENERIC_DEPLOYMENT_KEYS:
            bad.append({"model_id": sp.id, "reason": "generic", "deployment_key": dk})

    if bad:
        raise AppError(
            code="models_yaml_invalid",
            message="models.yaml deployment_key must be explicitly set (non-generic) for all models outside test profile",
            status_code=500,
            extra={
                "path": path,
                ...
                "bad_models": bad,
                "hint": (
                    "Set per-model deployment_key values (e.g. host-transformers--llama32-1b, "
                    "docker-llama--smollm2-360m-q8). To temporarily bypass: ALLOW_GENERIC_DEPLOYMENT_KEY=1"
                ),
            },
        )
...
def load_models_config() -> ModelsConfig:
    ...
    if path and os.path.exists(path):
        ...
        # Expand ${VAR} before profile selection & normalization
        raw = _expand_env(raw)
        ...
        norm_defaults: Dict[str, Any] = {
            ...
            "deployment_key": deployment_key_defaults,
            "readiness_mode": readiness_defaults,
        ...
        # Apply service-level enable flags based on primary model
        try:
        ...
        return ModelsConfig(
            ...
            defaults={
                ...
                # expose both for observability tooling
                "capabilities_raw": defaults_caps_raw,
                "capabilities": defaults_caps_bool,
                "deployment_key": deployment_key_defaults,
                "readiness_mode": readiness_defaults,
                "deployment": deployment_defaults,
                "assessment": assessment_defaults,
                "backend_constraints": backend_constraints_defaults,
                "transformers": norm_defaults.get("transformers") or {},
                "llamacpp": norm_defaults.get("llamacpp") or {},
                "remote": norm_defaults.get("remote") or {},
            },
        )
