# Documentation Index

## Start Here (Canonical)

- [Root README](../README.md): recruiter-first project overview and skill map
- [00) Testing and CI](00-testing.md)
- [01) Extraction Contract](01-extraction-contract.md)
- [02) Project Demos](02-project-demos.md)
- [03) Deployment Modes](03-deployment-modes.md)

## Deep Reference

- [04) Architecture Deep Dive](04-architecture-deep-dive.md)
- [05) Operations Runbook](05-operations-runbook.md)
- [06) Eval Methodology](06-eval-methodology.md)
- [07) ADR Index](07-adr-index.md)
- [08) Demo Evidence Index](08-demo-evidence-index.md)

## Archive (Historical)

Legacy docs were moved to [archive/](archive/).

Each archived file carries a warning banner:
- Historical snapshot; may not reflect current implementation.

Archive content is retained for project evolution context only.
